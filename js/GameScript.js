...
var valueGenerator = {
    "valueAdd" : {
        ...
        "laforcade": 4,
        "couland": 6,
        "walko": 1, //1 couland Per js
        "momo": 3, //3 couland Per js
        "mamie": 5 //5 couland Per js
    },
    "srcImage" : {
        ...
        "walko" : "images/walkoFace.png", //1 couland Per js
        "momo" : "images/momoFace.png", //3 couland Per js
        "mamie" : "images/mamieFace.png" //5 couland Per js
    }
};
var profGenerator  = [{
    "generator" : [{
...
function addUnit(nbToAdd) {
    playerStats.unit+=nbToAdd;
    stateButton();
}
function affUnit() {
...
function stateButton() {
    for(var i=0; i<getObjectLenght(currentCost.generator); i++){
        var nameprof = profGenerator[0]["generator"][0][i];
        var price = currentCost["generator"][nameprof];
        console.log(price);
        if(playerStats.unit<price)
            $('button[profValue="'+nameprof+'"]').attr("disabled","true");
        else
            $('button[profValue="'+nameprof+'"]').removeAttr("disabled");
    }

    for(var k=0; k<getObjectLenght(currentCost.generatorception); k++){
        var nameprof = profGenerator[0]["generatorception"][0][k];
        var price = currentCost["generatorception"][nameprof];
        if(playerStats.unit<price){
            $('button[profValue="'+nameprof+'"]').attr("disabled",true);
            console.log("n°: " + nameprof);
        }
        else
            $('button[profValue="'+nameprof+'"]').removeAttr("disabled");
    }
...
function loadSprite() {
    ...
    for(var i=0; i<getObjectLenght(playerStats.generator)-1; i++){
        var nameprof = profGenerator[0]["generator"][0][i]
        for(var k=0; k<playerStats.generator[nameprof]; k++){
            addSpriteGenerator(nameprof);
        }
    }

    for(var i=0; i<getObjectLenght(playerStats.generatorception)-1; i++){
        var nameprof = profGenerator[0]["generatorception"][0][i]
        for(var k=0; k<playerStats.generatorception[nameprof]; k++){
            addSpriteGenerator(nameprof);
        }
    }
}
...
function laborieAdd() {
    for(var i=0; i<playerStats.generator.laborie; i++)
        addUnit(valueGenerator.valueAdd.laborie);
}
function lafocadeAdd() {
    for(var i=0; i<playerStats.generator.laforcade; i++)
       addUnit(valueGenerator.valueAdd.laforcade);
}

function coulandAdd() {
    for(var i=0; i<playerStats.generator.couland; i++)
        addUnit(valueGenerator.valueAdd.couland);
}
function ernetAdd() {
    for(var i=0; i<playerStats.generator.ernet; i++)
        addUnit(valueGenerator.valueAdd.ernet);
}
