...
Personnage.prototype.dessinerPersonnage = function(context) {
	...
	if(this.etatAnimation >= DUREE_DEPLACEMENT) {
		// Si le déplacement a atteint ou dépassé le temps nécéssaire pour js'effectuer, on le termine
		this.etatAnimation = -1;
...
Personnage.prototype.deplacer = function(direction, map) {
	...
	if(prochaineCase.x < 0 || prochaineCase.y < 0 || prochaineCase.x >= map.getLargeur() || prochaineCase.y >= map.getHauteur()) {
		// On retourne un booléen indiquant que le déplacement ne js'est pas fait,
